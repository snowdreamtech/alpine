...
- **Server State**: Use **TanStack Query** (formerly React Query), **SWR**, or **Apollo Client** for fetching, caching, and mutating server state. Do not manage server state in Redux/Zustand.
...
- Use `React.FC<Props>` or prefer plain function definitions with typed props argument: `function UserCard({ userId }: UserCardProps)`.
- Use `ComponentPropsWithoutRef<'button'>` and `ComponentPropsWithRef<'input'>` for wrapping native HTML elements.
