...
- Use Open Graph meta tags (`og:title`, `og:description`, `og:image`) for pages shared on social media.

## 2. Semantic HTML

- Use semantic elements over generic `<div>` / `<span>`: `<header>`, `<nav>`, `<main>`, `<article>`, `<section>`, `<aside>`, `<footer>`, `<time>`, `<figure>`, `<figcaption>`.
- Use heading tags (`<h1>`–`<h6>`) in a logical, hierarchical order. Use **exactly one `<h1>`** per page — it is the primary heading.
- Use `<button>` for interactive controls (actions), `<a href>` for navigation (links). Never use `<div>` or `<span>` with click handlers as a substitute.
...
- Use ARIA roles and attributes (`aria-label`, `aria-describedby`, `aria-expanded`, `role`) **only when semantic HTML is insufficient**. The first rule of ARIA is: don't use ARIA.
...
- Avoid deprecated HTML elements (`<font>`, `<center>`, `<marquee>`, `<b>` for styling — use `<strong>` for semantics).
...
- Use `<link rel="canonical">` on every page to prevent duplicate content issues in search engines.
- Add `<meta name="robots">`, `rel="noindex"`, or `<link rel="canonical">` appropriately for pagination, filters, and user-generated content pages.
