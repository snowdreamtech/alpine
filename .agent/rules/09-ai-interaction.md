...
- **Mandatory Auto-Commit Logic**: When executing multiple tasks, suggestions, or changes in sequence, the AI agent **MUST** perform an automatic commit after each individual change — never batch them:
  1. **Implement** one change completely.
  2. **Auto-fix** (lint/format) the changed files.
  3. **Commit** it immediately with a precise Conventional Commit message.
  4. **Proceed** to the next item.

  This is a core operational requirement. Every feature implementation or bug fix must result in an immediate, independent, and auditable unit of work. Batching changes or skipping auto-commits is unacceptable.
