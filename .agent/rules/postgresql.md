...
- Always define a primary key. Prefer `BIGINT GENERATED ALWAYS AS IDENTITY` over `SERIAL`/`BIGSERIAL` for new tables.
...
- Always index foreign key columns — PostgreSQL does not create them automatically. Un-indexed FKs cause full table scans on joins and cascades.
...
- Monitor unused indexes: `SELECT * FROM pg_stat_user_indexes WHERE idx_scan = 0` and remove those not needed.

## 3. Querying

- Always use **parameterized queries** / prepared statements to prevent SQL injection.
- Use explicit column names in `SELECT` — avoid `SELECT *` in application code. It couples code to schema structure.
...
- Avoid schema changes inside long-running transactions — DDL acquires heavy locks (`ACCESS EXCLUSIVE`). Use `pg_try_advisory_lock` for coordinating concurrent migrations.
...
- Take regular backups with `pg_dump` or continuous archiving (**pgBackRest**, **WAL-G** with S3). Test restores quarterly. A backup never tested is not a backup.
- Enable **connection pooling** with **PgBouncer** (transaction mode) in front of PostgreSQL for high-concurrency workloads.
- Monitor with `pg_stat_statements` (enable extension), `pg_stat_activity`, and `pg_locks`. Set `log_min_duration_statement = 1000` to log slow queries exceeding 1 second.
