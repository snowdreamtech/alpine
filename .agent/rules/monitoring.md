...
- Add **business-level metrics** (orders processed, payments failed, user signups) alongside technical infrastructure metrics.
...
- Propagate **W3C Trace Context** headers (`traceparent`, `tracestate`) across all service and async boundaries. Do not use custom headers.
- Add meaningful span attributes: `http.method`, `db.statement` (sanitized — never include bind parameters), `messaging.system`, `error.message`.
- Set an appropriate **sampling rate** (e.g., 10% head-based sampling for high-volume traffic; 100% for errors and specific traced operations via tail-based sampling).

## 5. Alerting & Dashboards

- Alerts MUST be **actionable**: when an alert fires, an on-call engineer must know exactly what service is impacted and the immediate mitigation step.
